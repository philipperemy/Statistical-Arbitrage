...
for i = 1:50:length(spreads)
    Spread = spreads(i);
    [ profits, vols, sharpes, I ] = CrossValidationParallel( pp, Spread, @SimpleTradingStrategy, nstd_range, wts, wsize_range, 0 );
    I(:,4)          = sharpes;
    I(:,5)          = profits;
    I(:,6)          = vols;
    ...
    wsize           = W(end,1);
    nstd            = W(end,2);
    [~, balance_cum]= SimpleTradingStrategy( pp, Spread, 1, length(Spread.px), struct('wsize', wsize, 'wts', wts, 'nstd', nstd), 0, 0 );
