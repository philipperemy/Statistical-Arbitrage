...
\begin{figure}[H]
    ...
    \end{subfigure}
    \caption{Distributions of the cointegrated triples}
    %\label{fig:three graphs}
\end{figure}
...
One of the techniques to assess the performance of a strategy is to compare it to the standard Buy and Hold strategy where the holder buys the stock index at time $0$ and sell them at time $T$. \cite{gatev2006} also considered a bootstrap approach to generate random trading signals to assess the performance of a strategy over pure randomness. This approach is not discussed here since such a strategy has a negative expectation because of the trading costs.
...
\section{Volatility Modelling of the spread using model $\mathcal{M}_7$}
...
This part focuses on the result of the different strategies for the period Jan, 01 2008 - Dec, 31 2009. Of the possible triples candidates, 5967 passed the cointegration tests described in Algorithm \ref{formation_spread_algorithm}. Table \ref{20_selected_triples_simple_bands}, \ref{20_selected_triples_simple_bands_sv} and \ref{20_selected_triples_zscore} show the 20 tradable triples used in each strategy: Bollinger Bands with and without Stochastic Volatility modelling and Z-score. Each of this tuple has passed the correlation and the cointegration tests. In addition, it must exhibit a very high Sharpe ratio on $\mathcal{I}$ to be shortlisted. $\beta = (\beta_1, \beta_2, \beta_3)$ represents the cointegrating vector associated to the triple.
All the triples exhibit a very high Sharpe ratio on the in-sample period and a lower but still promising Sharpe ratio on the ou-sample period. 
...
\hline
\end{tabular}
\caption{The 20 selected triples for the strategy based on ZScore (2008-2009)}
...
\begin{tabular}{lllll}
\hline
Statistics  			& Bollinger & Z-score & Bollinger (SV) & SPX\\
\hline
\# of observations in the sample								    & 505       &505      && 505\\
\# of observations in the training window						& 169       &169      && 169\\
\# of days in the trading period								    & 336       &336      && 336\\
\# of triples in the trading period   							& 20        &20       && N.A.\\
\# of cointegrated spreads to analyze 				      & 5967      &5967     && N.A.\\
Average annualized return 										      & 60.11 \%  &78.06 \% && 12.92 \%\\
Annualized Sharpe Ratio 										        & 2.57      &3.07     && 0.41\\
Annualized Sharpe Ratio (with risk free)            & 2.51      &3.01     && 0.36\\
Largest daily return 											          & 5.81\%    &8.31 \%  && 10.78 \%\\
Lowest daily return 											          & -5.53\%   &-4.23\%  && -8.92 \%\\
Cumulative profit on the trading period             & 79.38\%   &103.88 \%&& 17.26 \% \\
Correlation with the market returns 							  & 0.0637    &-0.0302  && 1\\
Skewness of the returns														  & 0.489     &1.006    && 0.133\\
Kurtosis of the returns														  & 6.973     &11.863   && 6.421 \\
Maximum Drawdown  												          & 8.44\%    &6.32 \%  && 32.73 \%\\
\hline
\end{tabular}
...
\caption{Portfolios valuations on period Jan, 01 2008 - Dec, 31 2009 for the different strategies}
\label{fig:1}
\end{figure}
