...
\end{align*}
$\theta$ is enriched with the new parameters. Thus, we only need to modify the particle filter such that we draws two sets of particles (one for $X_t$ and one for $Z_t$) instead of one.
...
$x_t$ represents the volatility associated to $y_t$. The log-returns are not used here for a specific reason discussed later. Assume the standard stochastic volatility model,
