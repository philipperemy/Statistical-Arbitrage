...
$x_t$ represents the volatility associated to $y_t$. The log-returns are not used here for a specific reason discussed later. 

This is for normal leverage model. Recall that
\begin{align*}
y_t | x_t 						&= \mathcal{N}\left(\rho \beta \exp \left(\frac{x_t}{2} \right) \epsilon_{X,t}, \beta^2 \exp(x_t) (1-\rho^2)\right) \\
\frac{S_t}{S_{t-1}}-1 | x_t 	&= \mathcal{N}\left(\rho \beta \exp \left(\frac{x_t}{2} \right) \epsilon_{X,t}, \beta^2 \exp(x_t) (1-\rho^2)\right) \\
\frac{S_t}{S_{t-1}} | x_t 		&= \mathcal{N}\left(1 + \rho \beta \exp \left(\frac{x_t}{2} \right) \epsilon_{X,t}, \beta^2 \exp(x_t) (1-\rho^2)\right) \\
S_t | x_t 						&= \mathcal{N}\left( S_{t-1} + S_{t-1} \rho \beta \exp \left(\frac{x_t}{2} \right) \epsilon_{X,t}, S_{t-1}^2 \beta^2 \exp(x_t) (1-\rho^2)\right)
\end{align*}
The difference between the normal leverage model and the standard model is the correcting drift term $S_{t-1} \rho \beta \exp \left(\frac{x_t}{2} \right) \epsilon_{X,t}$. When $\rho = 0$, this term disappears.

This trick does not work for log-returns. The idea behind using these models is to catch the dynamics of the spread. A stock price is only one observation of a more general process over a time interval. Two general approaches can be derived.
The first approach consists in working with returns and not price curves like used usually with Bollinger bands. Denote $x_t = \sigma(t)$. The idea is to use a moving average on $\sigma(t)$ to compute the bands:
...
$N$ paths $\{S_{t,n}\}_{0 < n \leq N, t \in \mathbb{N}}$ are generated from the equations above. Let $f : \mathbb{R}^{+N} \rightarrow \mathbb{R}^+$ be a positive-definite aggregation function. The aggregated volatility of all the paths is defined as:

$$\sigma(t,p) = f(SMA_\sigma(t, p)_1,...,SMA_\sigma(t, p)_N)$$

If $f$ is simply the sample mean estimator, this becomes:

$$\sigma(t,p) = \frac{1}{N}\sum_{i=1}^N SMA_\sigma(t, p)_i$$

Note
$\sigma(t)$ is the limit of the rolling volatility of the prices when $p \rightarrow 1$.
...
$$SR =  \sqrt{252 }  . \frac{T^{-1} \sum_{t=1}^T R_t}{\sqrt{T^{-1} \sum_{t=1}^T (R_t -T^{-1} \sum_{t=1}^T R_t)^2 }} $$
...
\end{itemize}
The general method to find the optimal lag for the Phillips-Perron test is to begin with few lags, and then evaluate the sensitivity of the results by adding more lags. Another rule of thumb is to look at sample autocorrelations of $\Delta y_t = y_t - y_{t-1}$. Slow rates of decay require more lags. It is less pertinent for economic data because it is well known that the returns $\Delta y_t$ show no autocorrelation.\\
