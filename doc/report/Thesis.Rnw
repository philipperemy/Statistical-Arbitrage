...
For each $(\mathcal{I}_i)_{1 \leq i \leq 12}$, the cointegrated triples are formed, the models are calibrated and 20 tradable triples are selected based on the $\mathcal{SR}$ criteria. Then, both strategies are tested on the corresponding out-sample set $\mathcal{O}_i$ with the same models parameters. Finally, the results are confronted. \\

Table 1 shows the 20 tradable triples used in the strategy without SV modelling, for the period Jan, 01 2008 - Dec, 31 2009. Of the possible candidates, 5967 passed the cointegration tests described in Algorithm \ref{formation_spread_algorithm}. All the triples exhibit a very high Sharpe ratio on the in-sample period a much lower but still promising Sharpe ratio on the ou-sample period. 
...
\hline
\end{tabular}
\caption{Simple Bollinger Bands}
\end{table}
...
\begin{tabular}{lllll}
\hline
Summary Statistics of the triple strategy  			& Bollinger & Z-score & Bollinger (SV) & SPX\\
\hline Jan, 01 2008 - Dec, 31 2009 \\
\hline
