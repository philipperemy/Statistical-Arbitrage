...
From a mathematical point of view, the stratified resampling is compelling. A benchmark study consisting in resampling 1000 weights numerous times was performed and the results are presented in Table \ref{resampling_method_table}. The stratified resampling seems to offer the best balance in terms of speed and variance. For this reason, we choose it as default resampling method inside our particle filters.
