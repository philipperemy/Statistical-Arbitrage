...
\chapter{Procedure}
The workflow involves the preparation of the datasets and the selection of the cointegrated tuples. Then, the three strategies are applied and backtested on the data. Finally, the cumulative returns and some risk measures are presented to assess the performance of the strategies.
...
\end{equation}
$\hat{\mu}$ represents the mean asset return and $\hat{\sigma}$ the standard deviation of the returns. The idea of the Sharpe ratio is to quantify how much additional return the investor is receiving for the additional volatility of holding the risky asset over a risk-free asset. A variant is to subtract the daily risk free return $y_{rf}$, given by the US Treasury bills, to the returns of the strategy. This version is expected to output a slightly lower value. \textbf{A general rule of thumb is: a ratio of 1 or better is considered good, 2 and better is very good, and 3 and better is considered excellent.} \\
...
\subsubsection{HRSRI Portfolio Approach}
...
This thesis has explored the cointegration theory and applied it to financial datasets to detect cointegrated prices. These spread instruments have a mean-reverting property which is very important among Statistical Arbitrage strategies because it ensures that the prices will revert sufficiently often to their short-run mean. \\
