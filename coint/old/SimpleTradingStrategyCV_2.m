...
[ Spreads, beta, tuples_ret ] = SpreadConstructor(pp, stock_ids );

nstd_range = 2.0:0.1:3.0;
wts_range = 0:1;
wsize_range = 50:1:100;
...
for nstd = nstd_range
   for wts = wts_range
      for wsize = wsize_range
        ...
        pl = SimpleTradingStrategy2_UPDATED( Spreads, 1, 6000, boll_conf );
        cumsum_pl = cumsum(pl);
...
pl = SimpleTradingStrategy2_UPDATED( Spreads, 1, 6100, boll_conf );