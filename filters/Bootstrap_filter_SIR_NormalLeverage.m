...
function [estimated_states, return_weights, p_y, error] = Bootstrap_filter_SIR_NormalLeverage(y, rho, sigma, beta, N, steps, cor)
    w = zeros(N, steps);
    p = zeros(N, steps);
    p_y = zeros(1, steps);
    ...
    for i=1:steps
        ...
        if (i == FIRST_STEP)
            ...
            p(:,i) = rho * p_previous + sigma * (cor * innov_Y + sqrt(1-cor^2)*randn(N,1));
