...
function [log_p_y_given_theta, estimated_states] = BootstrapParticleFilter_SVMA1(y, rho, sigma, beta, psi, N, p_y_given_x)
    
    %fprintf('rho = %f, sigma = %f, beta = %f, psi = %f\n', rho, sigma, beta, psi);
    T = length(y);
    p = zeros(N, T);
    w = zeros(N, T);
    ...
    w(:,1)                  =   p_y_given_x(y(1), 0, beta*exp(0.5*p(:,1))); %not true but never mind
    ...
    for t = 2:T
        ...
        w(:,t)                  = p_y_given_x(y(t), 0, beta * exp(0.5*p(:,t)) + psi * beta * exp(0.5*p(nIdx,t-1)));
